module.exports = async function handler(req, res) {
  ...
  const { summary, sheetName } = req.body || {};
  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1200,
        system: `You are a senior performance marketing analyst for neOwn — India's largest children's book rental subscription service. Analyse Facebook and Google ad data. Use Indian number formatting (L, Cr) when appropriate. Be concise and direct.
...
Return ONLY a valid JSON object — no markdown, no backticks, no extra text:
{
  ...
  "aiCharts": [
    {"title": "chart idea", "insight": "what this metric would reveal"}
        ...
        messages: [
          { role: 'user', content: `Analyse this data for ${sheetName}:\n\n${summary}` }
        ],
      }),
    });
